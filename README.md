...
|   +-- mosaicSamples/              # One representative .mp4 per quality class (5 clips)
|
|-- Source/
...
- **[Samples/](https://github.com/CesarEmilioC/thesisRepo_A00830006/tree/main/Samples)** -- Contains 10 sample clips and their corresponding coordinate JSONs for testing and demonstration purposes. These samples cover different players and grade levels, and can be used to test pose estimation, visualization, and prediction without needing access to the full video dataset.
  ...
  - `mosaicSamples/`: Five video clips (one per quality class: grades 1, 3, 5, 8, 9) used to build the dataset mosaic figure in the thesis. Lets `thesisMosaic` run without needing access to the full `Videos/Clips/` dataset on OneDrive.
...
python main.py thesisMosaic \
  --clips-dir "../Samples/mosaicSamples" \
  --coords-dir "../Coordinates"
```
`Samples/mosaicSamples/` ships one clip per quality class (grades 1, 3, 5, 8, 9), which is everything `thesisMosaic` needs to reproduce the thesis figure without access to the full `Videos/Clips/` folder on OneDrive. Point `--clips-dir` at the full folder if you want to pick different representatives.
